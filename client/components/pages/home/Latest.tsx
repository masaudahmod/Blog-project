...
export default async function Latest() {
  ...
  const digestContent = contents.find((item: { section_key: string; content?: Record<string, string> }) => item.section_key === "daily-middle-ad");
  ...
  const digestTitle = digestContent?.content?.title || "World Wide trending articles";
  const digestDescription = digestContent?.content?.description || "Key takeaways generated by our model from 50+ sources today.";
  const digestCtaLabel = digestContent?.content?.subtitle || "";
  const digestCtaLink = digestContent?.content?.link || "";
